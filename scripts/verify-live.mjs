 ...
 *
 *   node scripts/verify-live.mjs https://staging.weave-it.org
 ...
 *
 * **No imports beyond node's own.** `watch.yml` runs this weekly without an
 * `npm ci`, which is what keeps the weekly check cheap and independent of
 * whether the tree currently installs. It reads `data/live-urls.txt` and calls
 * `fetch`, and that is all it is allowed to need — in particular it must not
 * import `site.config.ts`, which would drag in a TypeScript loader.
 */
 ...
 *
 * Derived from the shape of the path rather than from a list of section names,
 * so a new section needs no edit here: `/blog/` and `/blog/a-post/` are two
 * families because the rules that handle them are different, and every
 * `/tag/<x>/` is one family because one rule handles all 57.
 */
const familyOf = (u) => {
