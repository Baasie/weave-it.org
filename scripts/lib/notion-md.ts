 ...
 * No existing slug changes — they are all ASCII — and a name that is not a slug has no
...
/** HTML entities that survived the WordPress import.
 *
 * The importer converted HTML to markdown but did not decode character
 * references, so a handful of pages reached Notion with `&amp;`, `&#215;` and
 * `&#8211;` in their text — and Notion stores text, so it kept them faithfully.
 * They then render literally on the page: "Domain Driven Design &amp; Data
 * Mesh" is what a visitor reads.
 *
 * **This belongs in the sync, in the same way tag spelling does** (see
 * src/lib/tags.ts). It is one rule in one place, it is testable, and it applies
 * to an entity pasted in tomorrow as well as the ones the import left behind.
 * Notion stays the source of truth for the words; this decides that a character
 * reference is spelled as the character.
 *
 * Decoding is skipped inside inline code, where `&amp;` may well be the point.
 */
...
export function decodeEntities(s: string): string {
  return s.replace(/&(#x?[0-9a-f]+|[a-z]+);/gi, (whole, body: string) => {
    if (body.startsWith('#')) {
      ...
      // A reference outside the Unicode range, or one naming a control
      // character, is more likely a false positive than a character somebody
      // meant. Leaving it alone is the safe direction.
...
export function plainTitle(page: any, prop: string): string {
  ...
  return decodeEntities((p?.title ?? []).map((t: any) => t.plain_text).join('')).trim();
}
...
export function richText(rts: any[] = []): string {
  return rts.map((rt) => {
    ...
    let t = rt.plain_text ?? '';
    // Not inside code, where an entity may be the thing being shown.
    if (!a.code) t = decodeEntities(t);
    if (a.code) t = '`' + t + '`';
    if (a.bold) t = `**${t}**`;
    if (a.italic) t = `*${t}*`;
